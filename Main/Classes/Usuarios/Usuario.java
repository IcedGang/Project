    ...
    public Usuario searchTree(String nome, String cpf, Arvore<Usuario> tree){
        ...
        while(tree != null){
            ...
            if(this.getNome().equals(nome) && this.getCpf().equals(cpf)){
                x.setNome(node.getInfo().getNome());
                x.setCpf(node.getInfo().getCpf());
                x.setEndereco(node.getInfo().getEndereco());
                x.setCep(node.getInfo().getCep());
                x.setEmail(node.getInfo().getEmail());
                x.setComprovante(node.getInfo().getComprovante());
                x.setHist(node.getInfo().getHist()); 
            }
